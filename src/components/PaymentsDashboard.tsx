...
import { mockTherapists } from '../data/index.ts';
import type { Payment, PatientStatus, User } from '../types/index.ts';
...
export const PaymentsDashboard = ({ user, payments, setPayments }: PaymentsDashboardProps) => {
    ...
    const filteredPayments = useMemo(() => {
        let items = user.role === 'admin' 
            ? payments 
            : payments.filter(p => p.therapistId === user.id);
    ...
    }, [searchTerm, paymentStatusFilter, patientStatusFilter, payments, user]);

    return (
        <div className="payments-dashboard">
            <h3>{user.role === 'admin' ? 'ניהול כלל המטופלים' : 'המטופלים שלי'}</h3>
